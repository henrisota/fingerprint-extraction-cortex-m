...
static status_t FLEXCOMM_SetPeriph(FLEXCOMM_Type *base,
		FLEXCOMM_PERIPH_T periph, int lock);
...
static flexcomm_irq_handler_t s_flexcommIrqHandler[ARRAY_SIZE(
		s_flexcommBaseAddrs)];
...
static bool FLEXCOMM_PeripheralIsPresent(FLEXCOMM_Type *base,
	...
	if (periph == FLEXCOMM_PERIPH_NONE) {
		...
		return (base->PSELID & (1UL << ((uint32_t) periph + 3U))) > 0UL ?
				true : false;
...
static status_t FLEXCOMM_SetPeriph(FLEXCOMM_Type *base,
	...
	if (((base->PSELID & FLEXCOMM_PSELID_LOCK_MASK) != 0U)
			&& ((base->PSELID & FLEXCOMM_PSELID_PERSEL_MASK)
					!= (uint32_t) periph)) {
		return kStatus_Fail;
	}
...
void FLEXCOMM0_DriverIRQHandler(void) {
	...
	s_flexcommIrqHandler[instance]((uint32_t*) s_flexcommBaseAddrs[instance],
			s_flexcommHandle[instance]);
	SDK_ISR_EXIT_BARRIER;
}
#endif
