...
static void pllFindSel(uint32_t M, uint32_t *pSelP, uint32_t *pSelI,
		uint32_t *pSelR);
...
static pll_error_t CLOCK_GetPll0Config(uint32_t finHz, uint32_t foutHz,
		pll_setup_t *pSetup, bool useSS);
...
void CLOCK_SetClkDiv(clock_div_name_t div_name, uint32_t divided_by_value,
	...
	if ((div_name >= kCLOCK_DivFlexFrg0) && (div_name <= kCLOCK_DivFlexFrg7)) {
		...
		SYSCON_FLEXFRG0CTRL_DIV_MASK
				| SYSCON_FLEXFRG0CTRL_MULT(divided_by_value);
	} else {
		if (reset) {
		...
		if (divided_by_value == 0U) /*!<  halt */
		{
			...
			((volatile uint32_t*) pClkDiv)[(uint32_t) div_name] =
					(divided_by_value - 1U);
		}
	}
}
...
void CLOCK_SetRtc1khzClkDiv(uint32_t divided_by_value) {
	PMC->RTCOSC32K |= (((divided_by_value - 28U)
			<< PMC_RTCOSC32K_CLK1KHZDIV_SHIFT) | PMC_RTCOSC32K_CLK1KHZDIV_MASK);
}
...
void CLOCK_SetRtc1hzClkDiv(uint32_t divided_by_value) {
	if (divided_by_value == 0U) /*!<  halt */
	{
		...
		PMC->RTCOSC32K |=
				(((divided_by_value - 31744U) << PMC_RTCOSC32K_CLK1HZDIV_SHIFT)
						| PMC_RTCOSC32K_CLK1HZDIV_MASK);
	}
}
...
status_t CLOCK_SetupFROClocking(uint32_t iFreq) {
	...
	SYSCON->PRESETCTRLCLR[2] =
			(1UL << SYSCON_PRESETCTRL2_ANALOG_CTRL_RST_SHIFT);
...
void CLOCK_SetFLASHAccessCyclesForFreq(uint32_t iFreq) {
	...
	FLASH->DATAW[0] =
			(FLASH->DATAW[0] & 0xFFFFFFF0UL)
					| (num_wait_states
							& (SYSCON_FMCCR_FLASHTIM_MASK
									>> SYSCON_FMCCR_FLASHTIM_SHIFT));
	...
	SYSCON->FMCCR = (SYSCON->FMCCR & ~SYSCON_FMCCR_FLASHTIM_MASK)
			| ((num_wait_states << SYSCON_FMCCR_FLASHTIM_SHIFT)
					& SYSCON_FMCCR_FLASHTIM_MASK);
}
...
uint32_t CLOCK_GetUsb1ClkFreq(void) {
	return ((ANACTRL->XO32M_CTRL & ANACTRL_XO32M_CTRL_ENABLE_PLL_USB_OUT_MASK)
			!= 0UL) ? s_Ext_Clk_Freq : 0U;
}
...
uint32_t CLOCK_GetFro12MFreq(void) {
	return ((ANACTRL->FRO192M_CTRL & ANACTRL_FRO192M_CTRL_ENA_12MHZCLK_MASK)
			!= 0UL) ? 12000000U : 0U;
}
...
uint32_t CLOCK_GetFro1MFreq(void) {
	return ((SYSCON->CLOCK_CTRL & SYSCON_CLOCK_CTRL_FRO1MHZ_CLK_ENA_MASK) != 0UL) ?
			1000000U : 0U;
}
...
uint32_t CLOCK_GetExtClkFreq(void) {
	return ((ANACTRL->XO32M_CTRL & ANACTRL_XO32M_CTRL_ENABLE_SYSTEM_CLK_OUT_MASK)
			!= 0UL) ? s_Ext_Clk_Freq : 0U;
}
...
uint32_t CLOCK_GetWdtClkFreq(void) {
	return CLOCK_GetFro1MFreq()
			/ ((SYSCON->WDTCLKDIV & SYSCON_WDTCLKDIV_DIV_MASK) + 1U);
}
...
uint32_t CLOCK_GetFroHfFreq(void) {
	return ((ANACTRL->FRO192M_CTRL & ANACTRL_FRO192M_CTRL_ENA_96MHZCLK_MASK)
			!= 0UL) ? 96000000U : 0U;
}
...
uint32_t CLOCK_GetFlexCommClkFreq(uint32_t id) {
	...
	return (uint32_t) (((uint64_t) freq * ((uint64_t) frgDiv + 1ULL))
			/ (frgMul + frgDiv + 1UL));
}
	...
	case 0U:
		freq =
				CLOCK_GetCoreSysClkFreq()
						/ ((((volatile uint32_t*) pSystickClkDiv)[(uint32_t) id]
								& 0xffU) + 1U);
		break;
	case 1U:
		freq = CLOCK_GetFro1MFreq();
...
uint32_t CLOCK_SetFlexCommClock(uint32_t id, uint32_t freq) {
	...
	if ((freq > 48000000UL) || (freq > input) || (input / freq >= 2UL)) {
		...
		mul = (uint32_t) ((((uint64_t) input - freq) * 256ULL)
				/ ((uint64_t) freq));
...
static float findPll0MMult(void) {
	...
	if ((SYSCON->PLL0SSCG1 & SYSCON_PLL0SSCG1_SEL_EXT_MASK) != 0UL) {
		mMult = (float) (uint32_t) ((SYSCON->PLL0SSCG1
				& SYSCON_PLL0SSCG1_MDIV_EXT_MASK)
		...
		mMult_fract = ((float) (uint32_t) ((SYSCON->PLL0SSCG0)
				& PLL0_SSCG_MD_FRACT_M)
 ...
 */
static pll_error_t CLOCK_GetPll0ConfigInternal(uint32_t finHz, uint32_t foutHz,
		pll_setup_t *pSetup, bool useSS) {
	...
	if (useSS == false) {
		...
		pSetup->pllsscg[1] = (uint32_t) ((PLL_SSCG1_MDEC_VAL_SET(pllMultiplier))
				| (1UL << SYSCON_PLL0SSCG1_SEL_EXT_SHIFT));
	} else {
		uint64_t fc;
		...
		/* Set multiplier */
		pSetup->pllsscg[0] = (uint32_t) (PLL0_SSCG_MD_INT_SET(pllMultiplier)
				| PLL0_SSCG_MD_FRACT_SET((uint32_t )fc));
		pSetup->pllsscg[1] = (uint32_t) (PLL0_SSCG_MD_INT_SET(pllMultiplier)
				>> 32U);
	}
...
static pll_error_t CLOCK_GetPll0Config(uint32_t finHz, uint32_t foutHz,
	...
	s_PllSetupCacheIdx = (s_PllSetupCacheIdx + 1U)
...
pll_error_t CLOCK_SetupPLL0Data(pll_config_t *pControl, pll_setup_t *pSetup) {
	...
	/* PLL flag options */
	pllError = CLOCK_GetPll0Config(inRate, pControl->desiredRate, pSetup,
			useSS);
	if ((useSS) && (pllError == kStatus_PLL_Success)) {
		/* If using SS mode, then some tweaks are made to the generated setup */
		pSetup->pllsscg[1] |= (uint32_t) pControl->ss_mf
				| (uint32_t) pControl->ss_mr | (uint32_t) pControl->ss_mc;
...
pll_error_t CLOCK_SetupPLL0Prec(pll_setup_t *pSetup, uint32_t flagcfg) {
	...
	SYSCON->PLL0SSCG1 = pSetup->pllsscg[1]
			| (1UL << SYSCON_PLL0SSCG1_MREQ_SHIFT)
			| (1UL << SYSCON_PLL0SSCG1_MD_REQ_SHIFT); /* latch */
...
pll_error_t CLOCK_SetPLL0Freq(const pll_setup_t *pSetup) {
	...
	SYSCON->PLL0SSCG1 = pSetup->pllsscg[1];
	SYSCON->PLL0SSCG1 = pSetup->pllsscg[1]
			| (1UL << SYSCON_PLL0SSCG1_MD_REQ_SHIFT)
			| (1UL << SYSCON_PLL0SSCG1_MREQ_SHIFT); /* latch */
...
void CLOCK_SetupPLL0Mult(uint32_t multiply_by, uint32_t input_freq) {
	...
	SYSCON->PLL0CTRL = SYSCON_PLL0CTRL_CLKEN_MASK
			| SYSCON_PLL0CTRL_BYPASSPOSTDIV(0)
			| SYSCON_PLL0CTRL_BYPASSPOSTDIV2(0)
			| (selr << SYSCON_PLL0CTRL_SELR_SHIFT)
			| (seli << SYSCON_PLL0CTRL_SELI_SHIFT)
	...
	SYSCON->PLL0SSCG1 = mdec | (1UL << SYSCON_PLL0SSCG1_MREQ_SHIFT); /* select non sscg MDEC value, assert mreq and select mdec value */
}
...
bool CLOCK_EnableUsbfs0DeviceClock(clock_usbfs_src_t src, uint32_t freq) {
	...
	if (kCLOCK_UsbfsSrcFro == src) {
		...
		ANACTRL->FRO192M_CTRL | ANACTRL_FRO192M_CTRL_ENA_96MHZCLK_MASK
		...
		const pll_setup_t pll1Setup = { .pllctrl = SYSCON_PLL1CTRL_CLKEN_MASK
				| SYSCON_PLL1CTRL_SELI(19U) | SYSCON_PLL1CTRL_SELP(9U),
				.pllndec = SYSCON_PLL1NDEC_NDIV(1U), .pllpdec =
						SYSCON_PLL1PDEC_PDIV(5U), .pllmdec =
						SYSCON_PLL1MDEC_MDIV(30U), .pllRate = 48000000U,
...
bool CLOCK_EnableUsbfs0HostClock(clock_usbfs_src_t src, uint32_t freq) {
	...
	if (kCLOCK_UsbfsSrcFro == src) {
		...
		ANACTRL->FRO192M_CTRL = ANACTRL->FRO192M_CTRL
				| ANACTRL_FRO192M_CTRL_ENA_96MHZCLK_MASK;
		/* Select FRO 96 MHz */
		CLOCK_AttachClk(kFRO_HF_to_USB0_CLK);
	} else {
		/*!< Configure XTAL32M */
...
bool CLOCK_EnableUsbhs0PhyPllClock(clock_usb_phy_src_t src, uint32_t freq) {
	...
	if (ret) {
		USBPHY->PLL_SIC = (USBPHY->PLL_SIC & ~USBPHY_PLL_SIC_PLL_DIV_SEL(0x7))
				| phyPllDiv;
